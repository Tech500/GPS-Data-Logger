...
//
//
//   Upload              Note:  Must use esp32 core by ESP32 Community version 1.0.1 from "Arduino IDE, Board Manager"
//
///////////////////////////////////////////////////////////////////////////////////////////////////////
...
const char* loginIndex =
     ...
     "else"
     "{"
     " alert('Error Password or Username')/*displays error message*/"  //Set user and password on this line --for OTA.
     "}"
     "}"
     "</script>";
...
float pressure;  //Present pressure reading used to find pressure change difference.
...
//edit ThingSpeak.com data here...  You will need a free "ThingSpeak.com" account.
...
volatile int flag;  //For determing whether listen function is called or not called.
...
//////FTP Setup, ensure SPIFFS is started before ftp;  /////////
     ...
     if (SPIFFS.begin())
     {
          ...
          ftpSrv.begin("username","password");    //username, password for ftp.  
     }
////////////////////////////////////////////////////////////////
...
void loop()
{
     ...
     if ((MINUTE % 5 == 0) && (SECOND == 50))  //Reset rain5min variable every 5 MINUTES 11 SECOND.  //SECONDS offset needed to not interfere with 15 minute routing
     {
     ...
     if ((MINUTE % 15 == 0) && (SECOND == 0))  //Write Data to "LOG.TXT" every 15 MINUTES.
     {
          ...
          //FTP_UPLOAD();  //This hangs somewhere in doFTP /////////////////////////////////////////////////////////////////////////////////////////////////////////////
          delayTime = 200;
          rain5min         = 0;
          //STORE RAINCOUNT IN EEPROM
...
void getWeatherData()
{
     ...
     pressure = pres + 30.49;  //Correction for relative pressure.  Relative pressure in millibars = Absoute pressure + elevation in meters/ 8.3
...
float updateDifference()  //Pressure difference for fifthteen minute interval
{
     ...
     //First loop pass pastPressure and pressure are equal resulting in an incorrect difference result.
     //Future loop passes difference results are correct

     Serial.println("currentPressure:  " + (String)pressure);

     difference = pressure - pastPressure;  //This will be pressure from this pass thru loop, pressure1 will be new pressure reading next loop pass
...
void webInterface()
{
	...
	//Make no changes; changes must match php file.
     ...
     if(WiFi.status()== WL_CONNECTED)
     {
          ...
          http.begin("http://yourHostedDomain/dataCollector.php");      //Specify request destination.  Edit with your Hosted Domain.
